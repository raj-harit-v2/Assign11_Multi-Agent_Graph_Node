...
# Load JSON results
perception_data = json.loads((project_root / "perception_results.json").read_text())
decision_data = json.loads((project_root / "decision_results.json").read_text())

# Create session
session = AgentSession("test-session-001", "Find number of BHK variants available in DLF Camelia from local sources.")
...
step3.perception = PerceptionSnapshot(
    ...
    last_tooluse_summary="Summary step completed successfully",
